...
let CommunicationLayer: any, LangExecutorFactory: any, LspConnectionFactory: any, DiagnosticAdapterFactory: any;
try {
...
const runtimeTests: TestDefinition[] = [
...
function parseScenarioArgs(): string[] {
  const args = process.argv.slice(2);
  return args.map(arg => arg.toLowerCase());
}

function shouldRunScenario(testDef: TestDefinition, selectedScenarios: string[]): boolean {
  if (!selectedScenarios || selectedScenarios.length === 0) return true; // Run all if no filter

  return selectedScenarios.some(selected => {
    const sel = selected.toLowerCase();

    // Match by language
    if (testDef.language && testDef.language.toLowerCase() === sel) return true;

    // Match by scenario
    if (testDef.scenario && testDef.scenario.toLowerCase() === sel) return true;

    // Match by communication layer type
    if (testDef.type && testDef.type.toLowerCase() === sel) return true;

    // Match by server (communication layer variant)
    if (testDef.server && testDef.server.toLowerCase() === sel) return true;

    // Special handling for communication layer aliases
    if (sel === 'lsp' && testDef.type === 'lsp') return true;
    if (sel === 'runtime' && testDef.type === 'runtime') return true;

    // Match by partial name in test name (for backward compatibility)
    if (testDef.name.toLowerCase().includes(sel)) return true;
...
async function runLspTest(testDef: TestDefinition, repoRoot: string): Promise<{ ok: boolean; logPath?: string; error?: string }> {
...
async function runNimsuggestTest(repoRoot: string, filePath: string, expectedBlocks: number): Promise<CommunicationResult> {
    ...
    proc.on('close', (code) => {
      ...
      if (vivafolioLines.length >= expectedBlocks) {
        resolve({
          ...
          }).filter(block => block !== null) as VivafolioBlockNotification[]
        });
      } else {
        resolve({
          success: false,
...
async function runRuntimeTest(testDef: TestDefinition, repoRoot: string): Promise<{ ok: boolean; logPath?: string; error?: string }> {
  const filePath = path.resolve(repoRoot, testDef.file);
...
async function run(): Promise<void> {
  ...
  const results: Array<{ name: string; ok: boolean; logPath?: string; error?: string; skipped?: boolean }> = [];
  let selectedCount = 0;
  ...
  for (const testDef of allTests) {
    ...
    let result: { ok: boolean; logPath?: string; error?: string };
    if (testDef.type === 'lsp') {
