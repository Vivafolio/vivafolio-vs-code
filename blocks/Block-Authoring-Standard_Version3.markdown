...
Every block must include a `block-metadata.json` file (or `package.json` with equivalent fields) at its root, defining its identity, type, schema, and resources.
