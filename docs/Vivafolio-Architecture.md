...
---
flowchart LR
    VSCode --> |updated block| Loader[Block Loader & Hooks <br> `packages/block-loader/`]
   

    subgraph Host[Vivafolio core]
        Indexer --> |entity graph| Graph[(Entity graph<br>Block Protocol)]
        
        Graph --> BlockManager[Block Manager]
        RuntimeAdapters[Runtime & LSP adapters <br> `mocklang-extension/`, <br> `test/runtime-path/`]
        subgraph BlockManaging
            BlockManager[BlockManager]
            BlockManager[BlockManager] --> BlocksRepo[Blocks & Frameworks <br> `blocks/`, `packages/block-frameworks/`] 
            BlockManager[BlockManager] --> Loader
...
| --- | --- | --- |
| Host extension | Activates commands, manages webviews, mediates protocol messages. | `src/extension.ts`, `src/host/*` |
...
| Auxiliary apps & tests | Validate architecture with end-to-end flows. | `apps/blockprotocol-poc/`, `vivafolio/test/scenarios/*.js` |
...
flowchart TB
    subgraph HostExt[Host Extension <br> `src/extension.ts`]
        ...
        Diagnostics[Diagnostics listener <br> `src/diagnostics/*`]
        IndexClient[Indexing service client <br> `src/host/indexingServiceClient.ts`]
        BlockManager[Block manager <br> `src/host/blockManager.ts`]
        WebviewPool[Webview lifecycle <br> `src/host/webviews/*`]
    end
    LoaderAPI["@vivafolio/block-loader"]

    Activation --> Diagnostics
    Activation --> IndexClient
    Diagnostics --> BlockManager
    IndexClient --> BlockManager
    BlockManager --> WebviewPool
    BlockManager -->|load resources| LoaderAPI
```
...
- **Block manager:** coordinates `@vivafolio/block-loader` to fetch bundles, assembles initial HTML with embedded entity data, and routes messages.
- **Webview pool:** ensures webviews are sandboxed, hydrated, and disposed, and keeps protocol logging in `vivafolio/test/logs/`.
...
flowchart LR
    ...
    Persistence["Persistence bridge <br> `src/updateWorkspace.ts`"]
...
- Test scaffolding validates block lifecycle via Playwright (`vivafolio/test/scenarios/basic-comms.js`) and VS Code UI automation (`test/wdio/README.md`).

### 3.4 Runtime & LSP Adapters
...
- **Protocol Logging:** Each block session writes LSP traffic and block protocol messages to `vivafolio/test/logs/`.
...
graph LR
    ...
    Log[Protocol logs <br> `vivafolio/test/logs/`]
    Diagnostics[VS Code Diagnostics]

    Block --> Updates --> HostService[Host services]
