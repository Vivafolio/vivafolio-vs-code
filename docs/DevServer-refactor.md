...
- **Legacy milestone scenarios** that are not needed for target UX: `hello-world`, `custom`, `nested-kanban`, `multi-view-sync`, `iframe-webviews`, `feature-showcase-block`, `solidjs-task-baseline`, `framework-compilation-demo`, `cross-framework-nesting`, `person-chip-example`, `table-view-example`, `board-view-example`. (The previously listed `resource-loader` and `custom-element-baseline` scenarios have now been removed from `apps/blockprotocol-poc/src/server.ts` and related assets.) They encode mock graphs and manual applyUpdate logic rather than exercising the IndexingService + Block Loader contract.
