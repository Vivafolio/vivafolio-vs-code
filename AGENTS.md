...
- Test runner:
  - Suite definitions and implementation paths:
    - Basic Communications: implemented in `test/scenarios/basic-comms.js` (run via `just test-scenario-basic-comms` or `npm run test:scenario:basic-comms`).
    - Call-site Diagnostics: implemented in `test/scenarios/callsite-diagnostics.js` (run via `just test-scenario-callsite-diagnostics` or `npm run test:scenario:callsite-diagnostics`).
    - Minimal connectivity (optional direct script): implemented in `test/e2e-connectivity.js`.
  - All sessions log LSP protocol to `test/logs/` with per-run timestamps.
...
- For test writing/maintenance guidelines, see `test/AGENTS.md`.
